...
class PresetRecognizer:
    """
    Classe para reconhecimento de presets de câmera usando KAZE e feature matching.
    Pipeline: pré-processamento -> KAZE -> Lowe ratio test
    """
    def __init__(
        self,
        ...
        """
        Inicializa o reconhecedor de presets usando KAZE.

        Args:
            num_features: número máximo de features KAZE por imagem.
        ...
        # Matcher para KAZE (usa L2 pois são descritores float)
    ...
    def _compute_descriptors(
        ...
        try:
            ...
            keypoints = self.kaze.detect(imagem, None)
            
            # Sort by response and keep top N
            keypoints = sorted(keypoints, key=lambda x: x.response, reverse=True)[:self.num_features]
            
            # Compute descriptors
            keypoints, descriptors = self.kaze.compute(imagem, keypoints)
            
        except Exception as e:
            logger.warning(f"Falha ao detectar/calcular descritores KAZE: {e}")
            return [], None
    ...
    def identificar_preset(
        ...
        for nome_preset, (keypoints_preset, descriptors_preset) in list(
            ...
            logger.debug(
                f"{nome_preset}: {num_matches} good matches"
            )
        ...
        logger.info(
            f"Melhor preset: {melhor_preset} "
            f"com {melhor_score} good matches"
        )
        if melhor_score >= self.min_good_matches:
            print(
                f"Preset identificado: {melhor_preset} "
