...
class PresetRecognizer:
    """
    Classe para reconhecimento de presets de câmera usando SIFT e feature matching.
    Pipeline: pré-processamento -> SIFT -> Lowe ratio test
    """
    def __init__(
        self,
    ...
    def _compute_descriptors(
        ...
        if imagem is None:
            return [], None
        try:
            keypoints, descriptors = self.sift.detectAndCompute(imagem, None)
        except Exception as e:
            logger.warning(f"Falha ao detectar/calcular descritores SIFT: {e}")
            return [], None
